...
export default function Workers({ searchQuery: externalSearchQuery, onSearchChange, exportTrigger }: WorkersProps) {
  ...
  const paginatedWorkers = useMemo(() => {
    if (recordsPerPage === 'ALL') return filteredWorkers;
    const pageSize = typeof recordsPerPage === 'number' ? recordsPerPage : 10;
    const start = (currentPage - 1) * pageSize;
    return filteredWorkers.slice(start, start + pageSize);
  }, [filteredWorkers, recordsPerPage, currentPage]);

  const totalPages = useMemo(() =>
    recordsPerPage === 'ALL' ? 1 : Math.ceil(filteredWorkers.length / (typeof recordsPerPage === 'number' ? recordsPerPage : 10)),
    [filteredWorkers, recordsPerPage]);
