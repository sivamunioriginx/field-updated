...
export interface CategoryBookingItem {
  categoryId: number;
...
export default function CategoryAnalysis({ onCategorySelect }: CategoryAnalysisProps) {
  const [data, setData] = useState<CategoryBookingItem[]>([]);
  ...
  const buildUrl = useCallback(() => {
    ...
    return qs
      ? `${API_ENDPOINTS.ADMIN_ANALYSIS_CATEGORY_BOOKINGS}?${qs}`
      : API_ENDPOINTS.ADMIN_ANALYSIS_CATEGORY_BOOKINGS;
  }, [fromDate, toDate]);
    ...
    return (
      <View style={styles.loadingContainer}>
        <View style={styles.loadingCard}>
          ...
          <Text style={styles.loadingSubtext}>Fetching category insights</Text>
        </View>
      </View>
    );
  }
    ...
    >
        ...
        <View style={styles.heroRow}>
          <View style={styles.heroContent}>
            ...
            <View style={styles.heroText}>
              <Text style={styles.heroTitle}>Category Analysis</Text>
              <Text style={styles.heroSubtitle}>Booking performance by category</Text>
            </View>
          </View>
      ...
      <View style={styles.chartCard}>
        <View style={styles.chartHeader}>
          <Ionicons name="bar-chart" size={22} color="#6366f1" />
        ...
        {data.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Category or booking data will appear here once available.</Text>
          </View>
        ) : (
          <View style={styles.chartWrap}>
            {data.map((item, index) => {
              ...
              return onCategorySelect ? (
                <TouchableOpacity key={item.categoryId} onPress={() => onCategorySelect(item.categoryId, item.categoryTitle)} activeOpacity={0.7}>
                  {row}
                </TouchableOpacity>
              ) : (
                <View key={item.categoryId}>{row}</View>
              );
            })}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  statCard1: { borderLeftWidth: 4, borderLeftColor: '#6366f1' },
