...
export default function Quotes({ searchQuery: externalSearchQuery, onSearchChange, exportTrigger }: QuoteProps) {
  ...
  const fetchQuotes = async () => {
    try {
      ...
      console.log('📡 Fetching quotes from:', API_ENDPOINTS.ADMIN_QUOTES);
      const response = await fetch(API_ENDPOINTS.ADMIN_QUOTES);
      ...
      if (data.success) {
        ...
        console.log('✅ Quotes fetched:', quotesData.length, 'records');
  ...
  const getFilteredAndSortedQuotes = () => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(Quote => 
        ...
        Quote.mobile.includes(query) ||
        (Quote.email && Quote.email.toLowerCase().includes(query)) ||
        (Quote.work_description && Quote.work_description.toLowerCase().includes(query)) ||
        (Quote.location && Quote.location.toLowerCase().includes(query)) ||
        (Quote.created_at && Quote.created_at.toLowerCase().includes(query))
      );
    }
    ...
    if (fromDate.trim() || toDate.trim()) {
      filtered = filtered.filter(q => {
        ...
        const qDay = formatDateToYYYYMMDD(qDate);
        if (fromDate.trim() && qDay < fromDate.trim()) return false;
        if (toDate.trim() && qDay > toDate.trim()) return false;
        return true;
      });
    }
  ...
  const getPaginatedQuotes = () => {
    const filtered = getFilteredAndSortedQuotes();
  ...
  const totalPages = (recordsPerPage === 'ALL' || hasDateFilter) 
    ? 1 
    : Math.ceil(getFilteredAndSortedQuotes().length / (typeof recordsPerPage === 'number' ? recordsPerPage : 10));
  ...
  if (quotes.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyText}>There are no quotes available at the moment.</Text>
      </View>
    );
  }
      ...
      <View style={styles.customersContainer}>
          ...
          >
            <View style={styles.tableContainer}>
              ...
              <View style={styles.tableHeader}>
                ...
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 90 : isTablet ? 60 : 50 }]}>
                  <Ionicons name="person" size={isDesktop ? 16 : 14} color="#ffffff" />
                  <Text style={styles.tableHeaderText}>Name</Text>
                </View>
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 130 : isTablet ? 130 : 120 }]}>
                  <Ionicons name="call" size={isDesktop ? 16 : 14} color="#ffffff" />
                  <Text style={styles.tableHeaderText}>Mobile</Text>
                </View>
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 300 : isTablet ? 200 : 180 }]}>
                  <Ionicons name="clipboard" size={isDesktop ? 16 : 14} color="#ffffff" />
                  <Text style={styles.tableHeaderText}>Work Description</Text>
                </View>
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 300 : isTablet ? 350 : 300 }]}>
                  <Ionicons name="home" size={isDesktop ? 16 : 14} color="#ffffff" />
                  <Text style={styles.tableHeaderText}>Address</Text>
                </View>
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 200 : isTablet ? 50 : 25 }]}>
                  <Ionicons name="location" size={isDesktop ? 16 : 14} color="#ffffff" />
                  <Text style={styles.tableHeaderText}>Quoted On</Text>
                </View>
              </View>
              ...
              {paginatedQuotes.map((quote, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View style={[styles.tableCell, { width: isDesktop ? 90 : isTablet ? 60 : 50 }]}>
                      <Text style={styles.tableCellText}>{quote.name || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 140 : isTablet ? 130 : 120 }]}>
                      <Text style={styles.tableCellText}>{quote.mobile || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 300 : isTablet ? 200 : 180 }]}>
                      <Text style={styles.tableCellText}>{quote.work_description || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 300 : isTablet ? 350 : 300 }]}>
                      <Text style={styles.tableCellText}>{quote.location || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 200 : isTablet ? 70 : 35  }]}>
                      <Text style={styles.tableCellText}>{formatDate(quote.created_at) || 'N/A'}</Text>
                    </View>
                  </View>
                );
              })}
        ...
        <View style={styles.paginationContainer}>
          <Text style={styles.paginationInfo}>
            {recordsPerPage === 'ALL' 
              ? `Showing all ${filteredQuotes.length} entries`
              : (() => {
                  const pageSize = typeof recordsPerPage === 'number' ? recordsPerPage : 10;
                  const start = ((currentPage - 1) * pageSize) + 1;
                  const end = Math.min(currentPage * pageSize, filteredQuotes.length);
                  return `Showing ${start} to ${end} of ${filteredQuotes.length} entries`;
                })()
            }
          </Text>
