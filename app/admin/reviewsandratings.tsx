...
export default function ReviewsRatings({ searchQuery: externalSearchQuery, onSearchChange }: ReviewsRatingsProps) {
  ...
  const fetchReviewsRatings = async () => {
    try {
      ...
      console.log('📡 Fetching reviews and ratings from:', API_ENDPOINTS.ADMIN_REVIEWS_RATINGS);
      const response = await fetch(API_ENDPOINTS.ADMIN_REVIEWS_RATINGS);
      ...
      console.log('📦 Reviews and ratings response:', data);
      
      if (data.success) {
        const reviewsData = data.reviews || data.data || [];
        console.log('✅ Reviews and ratings fetched:', reviewsData.length, 'records');
        setReviews(reviewsData);
        setCurrentPage(1);
      } else {
        console.error('❌ Failed to fetch reviews and ratings:', data.message);
        setReviews([]);
      }
    } catch (error) {
  ...
  const getFilteredAndSortedReviews = () => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(review => 
        ...
        review.worker_name?.toLowerCase().includes(query) ||
        review.booking_for?.toLowerCase().includes(query) ||
  ...
  const totalPages = recordsPerPage === 'ALL' 
    ? 1 
    : Math.ceil(getFilteredAndSortedReviews().length / (typeof recordsPerPage === 'number' ? recordsPerPage : 10));
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading Reviews & Ratings...</Text>
      </View>
    );
  }
  ...
  if (reviews.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyTitle}>No Reviews & Ratings Found</Text>
        <Text style={styles.emptyText}>There are no reviews and ratings available at the moment.</Text>
      </View>
    );
  }
      ...
      <View style={styles.reviewsContainer}>
        ...
        <View style={styles.tableWrapper}>
          <View style={styles.tableControlsRow}>
            {/* Sort and Records - aligned with table header */}
          ...
          >
            <View style={styles.tableContainer}>
              ...
              <View style={styles.tableHeader}>
                ...
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 130 : isTablet ? 100 : 80 }]}>
                  <Ionicons name="receipt-outline" size={isDesktop ? 16 : 14} color="#ffffff" />
                  <Text style={styles.tableHeaderText}>Booking ID</Text>
                </View>
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 180 : isTablet ? 140 : 100 }]}>
                ...
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 280 : isTablet ? 180 : 120 }]}>
              ...
              {paginatedReviews.map((review, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View style={[styles.tableCell, { width: isDesktop ? 180 : isTablet ? 100 : 80 }]}>
                      <Text style={styles.tableCellText}>{review.booking_id || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 180 : isTablet ? 140 : 100 }]}>
                      <Text style={styles.tableCellText}>{review.customer_name || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 180 : isTablet ? 140 : 100 }]}>
                      <Text style={styles.tableCellText}>{review.worker_name || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 250 : isTablet ? 180 : 120 }]}>
                      <Text style={styles.tableCellText} numberOfLines={2}>{review.booking_for || 'N/A'}</Text>
                    </View>
                    ...
                    <View style={[styles.tableCell, { width: isDesktop ? 300 : isTablet ? 200 : 150 }]}>
                      <Text style={styles.tableCellText} numberOfLines={2}>{review.review || 'N/A'}</Text>
                    </View>
                    <View style={[styles.tableCell, { width: isDesktop ? 200 : isTablet ? 150 : 120 }]}>
                      <Text style={styles.tableCellText}>{formatDate(review.created_at) || 'N/A'}</Text>
                    </View>
                  </View>
                );
              })}
        ...
        <View style={styles.paginationContainer}>
          <Text style={styles.paginationInfo}>
            {recordsPerPage === 'ALL' 
              ? `Showing all ${filteredReviews.length} entries`
              : (() => {
                  const pageSize = typeof recordsPerPage === 'number' ? recordsPerPage : 10;
                  const start = ((currentPage - 1) * pageSize) + 1;
                  const end = Math.min(currentPage * pageSize, filteredReviews.length);
                  return `Showing ${start} to ${end} of ${filteredReviews.length} entries`;
                })()
            }
          </Text>
...
const createStyles = (width: number, height: number) => {
  ...
  return StyleSheet.create({
    ...
    tableWrapper: {
      marginBottom: 12,
      marginLeft: isDesktop ? 10 : isTablet ? 130 : 110,
    },
    tableScrollView: {
      width: '100%',
    },
