...
export default function Categories({ searchQuery: externalSearchQuery, onSearchChange, exportTrigger }: CategoriesProps) {
  ...
  const getFilteredAndSortedCategories = () => {
    ...
    if (fromDate.trim() || toDate.trim()) {
      filtered = filtered.filter(cat => {
        const catDate = new Date(cat.created_at || 0);
        const catDay = formatDateToYYYYMMDD(catDate);
        if (fromDate.trim() && catDay < fromDate.trim()) return false;
        if (toDate.trim() && catDay > toDate.trim()) return false;
        return true;
      });
    }
  ...
  const totalPages = (recordsPerPage === 'ALL' || hasDateFilter) 
    ? 1 
    : Math.ceil(getFilteredAndSortedCategories().length / (typeof recordsPerPage === 'number' ? recordsPerPage : 10));
  ...
  if (categories.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={styles.emptyText}>There are no Categories available at the moment.</Text>
      </View>
    );
  }
      ...
      <View style={styles.customersContainer}>
          ...
          >
            <View style={styles.tableContainer}>
              ...
              <View style={styles.tableHeader}>
                ...
                <View style={[styles.tableCell, styles.tableHeaderCell, { width: isDesktop ? 350 : isTablet ? 70 : 35 }]}>
        ...
        <View style={styles.paginationContainer}>
          <Text style={styles.paginationInfo}>
            {recordsPerPage === 'ALL' 
              ? `Showing all ${filteredCategories.length} entries`
              : (() => {
                  const pageSize = typeof recordsPerPage === 'number' ? recordsPerPage : 10;
                  const start = ((currentPage - 1) * pageSize) + 1;
                  const end = Math.min(currentPage * pageSize, filteredCategories.length);
                  return `Showing ${start} to ${end} of ${filteredCategories.length} entries`;
                })()
            }
          </Text>
