...
export interface SubcategoryBookingItem {
...
interface SubcategoryAnalysisProps {
  ...
  onSubcategorySelect?: (subcategoryId: number, subcategoryTitle: string) => void;
}

export default function SubcategoryAnalysis({ categoryId, categoryTitle, onClearCategoryFilter, onSubcategorySelect }: SubcategoryAnalysisProps) {
  const [data, setData] = useState<SubcategoryBookingItem[]>([]);
  ...
  const buildUrl = useCallback(() => {
    ...
    return qs
      ? `${API_ENDPOINTS.ADMIN_ANALYSIS_SUBCATEGORY_BOOKINGS}?${qs}`
      : API_ENDPOINTS.ADMIN_ANALYSIS_SUBCATEGORY_BOOKINGS;
  ...
  const topSubcategory = data.length
    ...
    return (
      <View style={styles.loadingContainer}>
        <View style={styles.loadingCard}>
          ...
          <Text style={styles.loadingSubtext}>Fetching subcategory insights</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <ScrollView
      ...
      refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#64748b']} tintColor="#64748b" />
      }
      showsVerticalScrollIndicator={false}
    >
      {/* Hero header with gradient */}
      <LinearGradient
        colors={['#475569', '#64748b', '#334155']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        style={styles.hero}
      >
        <View style={styles.heroRow}>
          <View style={styles.heroContent}>
            ...
            <View style={styles.heroText}>
              <Text style={styles.heroTitle}>Subcategory Analysis</Text>
              <Text style={styles.heroSubtitle}>
                {categoryTitle ? `Subcategories under "${categoryTitle}"` : 'Booking performance by subcategory'}
              </Text>
            </View>
          </View>
          {categoryTitle && onClearCategoryFilter && (
            <TouchableOpacity style={styles.filterChip} onPress={onClearCategoryFilter} activeOpacity={0.8}>
              <Text style={styles.filterChipText}>Viewing: {categoryTitle}</Text>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={[styles.statCard, styles.statCard2]}>
          ...
          <Text style={styles.statValue} numberOfLines={1}>
            {topSubcategory?.subcategoryTitle || '—'}
          </Text>
          <Text style={styles.statLabel}>Top Subcategory</Text>
        </View>
        <View style={[styles.statCard, styles.statCard3]}>
          <View style={[styles.statIconWrap, styles.statIcon3]}>
            <Ionicons name="grid" size={24} color="#14b8a6" />
          </View>
      ...
      <View style={styles.chartCard}>
        <View style={styles.chartHeader}>
          <Ionicons name="bar-chart" size={22} color="#64748b" />
        ...
        {data.length === 0 ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Subcategory or booking data will appear here once available.</Text>
          </View>
        ) : (
          <View style={styles.chartWrap}>
            {data.map((item, index) => {
              ...
              return onSubcategorySelect ? (
                <TouchableOpacity key={item.subcategoryId} onPress={() => onSubcategorySelect(item.subcategoryId, item.subcategoryTitle)} activeOpacity={0.7}>
                  {row}
                </TouchableOpacity>
              ) : (
                <View key={item.subcategoryId}>{row}</View>
              );
            })}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  statCard1: { borderLeftWidth: 4, borderLeftColor: '#64748b' },
  statCard2: { borderLeftWidth: 4, borderLeftColor: '#f59e0b' },
  statCard3: { borderLeftWidth: 4, borderLeftColor: '#14b8a6' },
  statIconWrap: {
    width: 36,
    height: 36,
    borderRadius: 10,
  ...
  statIcon3: { backgroundColor: '#f0fdfa' },
  statValue: {
    fontSize: 18,
    fontWeight: '700',
    color: '#0f172a',
